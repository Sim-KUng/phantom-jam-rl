...
class Week12MetricsCallback(BaseCallback):
    """
    Week12 튜닝용 TensorBoard custom 지표 기록
    """
...
def main():
    print("[week12] 차선 변경/급감속 억제 튜닝 PPO 추가 학습을 시작합니다.")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_2lane_week12_tuned",
    )

    metrics_callback = Week12MetricsCallback()

    model.learn(
        total_timesteps=150_000,
        tb_log_name="ppo_2lane_week12_tuned",
        callback=[checkpoint_callback, metrics_callback],
        reset_num_timesteps=False,
        progress_bar=True,
    )

    final_model_path = "models/ppo_2lane_week12_tuned_final"
    model.save(final_model_path)

    env.close()

    print(f"[week12] 튜닝 학습 완료: {final_model_path}.zip")
